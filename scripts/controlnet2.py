...
class Script(scripts.Script):
    ...
    def show(self, is_img2img):
        # if is_img2img:
            # return False
        print("Checking show, return AlwaysVisible")
    ...
    def get_default_ui_unit_depth(self, is_ui=True):
    ...
    def get_default_ui_unit_pose(self, is_ui=True):
        cls = UiControlNetUnit if is_ui else external_code.ControlNetUnit
        return cls(
            enabled=True,
    ...
    def uigroup(self, tabname, is_img2img, elem_id_tabname):
        ...
        def svgPreprocess(inputs):
            if (inputs):
                print("in svgPreprocess, inputs:", list(inputs.keys()))
        ...
        print("create click fn controlnet_unit")
        if is_img2img:
            img2img_submit_button.click(fn=UiControlNetUnit, api_name="controlnet_unit", inputs=list(unit_args), outputs=unit, queue=False)
        else:
            txt2img_submit_button.click(fn=UiControlNetUnit, api_name="controlnet_unit", inputs=list(unit_args), outputs=unit, queue=False)

        print("uigroup returning unit:", unit) 
        return unit
    ...
    def ui(self, is_img2img):
        ...
        with gr.Group(elem_id=elem_id_tabname):
            with gr.Accordion("ControlNet", open = False, elem_id="controlnet"):
                print("ControlNet max_models:", max_models)
                if max_models > 1:
